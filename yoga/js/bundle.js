  ...
  function sendForm(formElement) {
    formElement.addEventListener('submit', function (event) {
      ...
      postData(formData).then(function () {
        return statusMassage.innerHTML = message.loading;
      }).then(function () {
        return statusMassage.innerHTML = message.success;
      }).catch(function () {
        return statusMassage.innerHTML = message.failure;
      }).then(clearInput);
    });
  }
...
function slide() {
  ...
  function showSlides(n) {
    ...
    }); // for(let i = 0; i < slides.length; i++){ //це то саме що зверху
